...
function display_cff( $feed_options ) {
    ...
    isset( $feed_options[ 'appid' ] ) && !empty( $feed_options[ 'appid' ] ) ? $cff_app_id = $feed_options[ 'appid' ] : $cff_app_id = '';
...
function cff_get_json_data( $feed_options, $next_urls_arr_safe = null, $data_att_html ) {
    ...
    if ($access_token == '' || !$cff_show_access_token) $access_token = $access_token_array[rand(0, 81)];
    ...
    foreach ( $page_ids as $page_id ) {
        ...
        if ($cff_events_only && $cff_events_source == 'eventspage'){
            ...
            //Events URL
            $cff_events_json_url = "https://graph.facebook.com/v2.9/".$page_id."/events/?fields=id,name,attending_count,cover,start_time,end_time,event_times,timezone,place,description,ticket_uri,interested_count".$cff_start_time_string.$cff_get_upcoming_events."&limit=200&access_token=" . $access_token . "&format=json-strings" . $cff_ssl;
            ...
            if($cff_past_events) $cff_events_json_url = 'https://graph.facebook.com/v2.9/'.$page_id.'/events?fields=name,id,attending_count,description,start_time,end_time,timezone,place,ticket_uri,cover,interested_count&limit=200&time_filter=past&access_token='.$access_token;

            //Group events
            if($cff_is_group && !$cff_past_events) $cff_events_json_url = 'https://graph.facebook.com/v2.9/' . $page_id . '/events?fields=name,id,description,start_time,end_time,event_times,timezone,ticket_uri,place,cover,attending_count,interested_count&limit=200&since='.$curtimeplus.'&access_token=' . $access_token;
        ...
        if (!$cff_events_only || ($cff_events_only && $cff_events_source == 'timeline') ){
            ...
            $cff_posts_json_url = 'https://graph.facebook.com/v2.9/' . $page_id . '/' . $graph_query . '?fields=id,from{name,id},message,message_tags,story,story_tags,picture,full_picture,link,source,name,caption,description,type,status_type,object_id,created_time,backdated_time,attachments{subattachments},shares,comments.summary(true){message,created_time},likes.summary(true).limit(0),call_to_action&access_token=' . $access_token . '&limit=' . $cff_post_limit . '&locale=' . $cff_locale . $cff_ssl . $cff_date_range;
        ...
        if ($cff_events_only && $cff_events_source == 'eventspage'){
            ...
            if( empty($json_data->data) ){
                ...
                foreach ($json_data->data as $event )
                {
                    ...
                    isset($event->place->location->country ) ? $venue_country = htmlentities($event->place->location->country, ENT_QUOTES | ENT_HTML5, 'UTF-8') : $venue_country = '';
                    ...
                    isset($event->ticket_uri) ? $ticket_uri = htmlentities($event->ticket_uri, ENT_QUOTES | ENT_HTML5, 'UTF-8') : $ticket_uri = '';
                    ...
                    $event_name = htmlentities($event_name, ENT_QUOTES | ENT_HTML5, 'UTF-8');
                    $location = htmlentities($location, ENT_QUOTES | ENT_HTML5, 'UTF-8');
                    $venue_street = htmlentities($venue_street, ENT_QUOTES | ENT_HTML5, 'UTF-8');
                    $venue_city = htmlentities($venue_city, ENT_QUOTES | ENT_HTML5, 'UTF-8');
                    $venue_state = htmlentities($venue_state, ENT_QUOTES | ENT_HTML5, 'UTF-8');
                    $venue_zip = htmlentities($venue_zip, ENT_QUOTES | ENT_HTML5, 'UTF-8');
            ...
            if($fbdata_string){
                ...
                foreach ($fbdata_string as $news)
                {
                    ...
                    if ($cff_post_type == 'link') {
                        isset($news->story) ? $story = htmlentities($news->story, ENT_QUOTES | ENT_HTML5, 'UTF-8') : $story = '';
                    ...
                    if( isset($news->status_type) ){
                        ...
                        if( $news->status_type == 'added_photos' || ($news->type == 'photo' && isset( $news->attachments->data[0]->subattachments )) ){
                            ...
                            //If the story contains a number, if it's a group post with attachments, or if the story doesn't contain a number but it has subattachments (eg: a shared post which has multiple photos)
                            if ( $num_photos > 1 || $is_group_post_with_attachments || ( $num_photos == 0 && isset( $news->attachments->data[0]->subattachments ) ) ) {
                    ...
                    $post_text = htmlentities($post_text, ENT_QUOTES | ENT_HTML5, 'UTF-8');
                    $description_text = htmlentities($description_text, ENT_QUOTES | ENT_HTML5, 'UTF-8');
                    ...
                    if ( $cff_show_post ) {
                        ...
                        if($cff_offset_show_post){
                            ...
                            if( isset($news->from->name) ){
                                $cff_author_name = htmlentities($news->from->name, ENT_QUOTES | ENT_HTML5, 'UTF-8');
                            ...
                            if( !$cff_photos_only && !$cff_videos_only && !($cff_albums_only && $cff_albums_source == 'photospage') && empty($cff_album_id) && !$cff_reviews ){
                                ...
                                if ( !empty($news->description) || !empty($news->caption) ) {
                                    ...
                                    if ( !empty($news->description) ) {
                                        $description_text = htmlentities($news->description, ENT_QUOTES | ENT_HTML5, 'UTF-8');
                                    } else {
                                        $description_text = htmlentities($news->caption, ENT_QUOTES | ENT_HTML5, 'UTF-8');
                                    }
                                ...
                                if ($cff_show_event_title || $cff_show_event_details) {
                                    ...
                                    if ($cff_post_type == 'event') {
                                        ...
                                        $event_json_url = 'https://graph.facebook.com/v2.12/'.$eventID.'?fields=cover,place,name,owner,start_time,timezone,id,comments.summary(true){message,created_time}&access_token=' . $access_token . $cff_ssl;
                                        ...
                                        if ($cff_show_event_details){
                                            ...
                                            if (!empty($news->description)){
                                                ...
                                                $description_safe = htmlentities($news->description, ENT_QUOTES | ENT_HTML5, 'UTF-8');
                                ...
                                if ($cff_post_type == 'link' || $cff_soundcloud) {
                                    ...
                                    if( $cff_soundcloud ){
                                        ...
                                        //The link title:
                                        if( isset($news->name) ) $cff_shared_link .= '"><'.$cff_link_title_format.' class="cff-link-title" '.$cff_link_title_styles.'><a href="'.$link.'" '.$target.$cff_nofollow.' style="color:#' . $cff_link_title_color . ';">'. htmlentities($news->name, ENT_QUOTES | ENT_HTML5, 'UTF-8') . '</a></'.$cff_link_title_format.'>';
                                        //The link source:
                                        (!empty($news->caption)) ? $cff_link_caption = htmlentities($news->caption, ENT_QUOTES | ENT_HTML5, 'UTF-8') : $cff_link_caption = '';
                                ...
                                if ($news->type == 'video' && !$cff_soundcloud) {
                                    ...
                                    //Else use a video player
                                        ...
                                        //Title & alt text
                                        isset( $news->name ) ? $vid_title = htmlentities($news->name, ENT_QUOTES | ENT_HTML5, 'UTF-8') : $vid_title = $cff_facebook_link_text;
                                        ...
                                        if( !$cff_disable_lightbox ){
                                            ...
                                            $cff_media_video .= '<a href="https://facebook.com/'.$cff_post_id.'" class="cff-html5-play"><i class="fa fa-play cff-playbtn" aria-hidden="true"></i><span class="cff-screenreader">Play</span></a>';
                                            ...
                                            if( $cff_video_action == 'facebook' ){
                                                ...
                                                if( isset( $news->source ) && $cff_mp4_check && $cff_video_player == 'standard' ){
                                                    ...
                                                    $cff_media_video .= '<a href="https://facebook.com/'.$cff_post_id.'" class="cff-html5-play"><i class="fa fa-play cff-playbtn" aria-hidden="true"></i></a>';
                                    ...
                                    if($cff_is_video_embed) {
                                        ...
                                        $cff_description .= '"><'.$cff_link_title_format.' class="cff-link-title" '.$cff_link_title_styles.'><a href="'.$link.'" '.$target.$cff_nofollow.' style="color:#' . $cff_link_title_color . ';">'. htmlentities($news->name, ENT_QUOTES | ENT_HTML5, 'UTF-8') . '</a></'.$cff_link_title_format.'>';
                                ...
                                if( $cff_show_meta || $cff_lightbox_comments ){
                                    ...
                                    if (!empty($news->comments->data)){
                                        ...
                                        foreach ($news->comments->data as $comment_item ) {
                                            $comment = htmlentities($comment_item->message, ENT_QUOTES | ENT_HTML5, 'UTF-8');
                                ...
                                //Start text wrapper
                                if ( ($cff_thumb_layout || $cff_half_layout) && (!empty($news->picture) || ($cff_post_type == 'event' && $cff_event_has_cover_photo) || ($news->status_type == 'created_note' && $cff_note_media_src) ) ){
                                    ...
                                    if($cff_show_media && $cff_media_position == 'above'){
                                        ...
                                        if( $news->status_type == 'created_note' ) $cff_media = $cff_note_picture_html;
                            ...
                            if( ($cff_albums_only && $cff_albums_source == 'photospage') && empty($cff_album_id) ){
                                ...
                                if ( $cff_filter_string != '' ){
                                    ...
                                    if ( cff_stripos_arr($cff_text_to_be_filtered, $cff_filter_strings_array) === false ) $cff_show_post = false;
                                }
                                ...
                                if ( $cff_exclude_string != '' ){
                                    ...
                                    if ( cff_stripos_arr($cff_text_to_be_filtered, $cff_exclude_strings_array) !== false ) $cff_show_post = false;
                                }

                                //Encode text after filtering is done
                                $cff_album_name = htmlentities($cff_album_name, ENT_QUOTES | ENT_HTML5, 'UTF-8');
                                $cff_album_description = htmlentities($cff_album_description, ENT_QUOTES | ENT_HTML5, 'UTF-8');
                            ...
                            if( $cff_album_active && !empty($cff_album_id) ){
                                ...
                                $cff_album_desc = htmlentities($cff_album_desc, ENT_QUOTES | ENT_HTML5, 'UTF-8');
                            ...
                            if($cff_videos_only && empty($cff_album_id)){
                                ...
                                //Encode text after filtering
                                $description_text = htmlentities($description_text, ENT_QUOTES | ENT_HTML5, 'UTF-8');
                                $video_name = htmlentities($video_name, ENT_QUOTES | ENT_HTML5, 'UTF-8');
                                ...
                                if( $cff_show_post ){
                                    ...
                                    $cff_post_item .= '<a href="https://facebook.com/' . $news->id . '" class="cff-album-cover cff-video" ' . $target . $cff_nofollow . ' id="' . $news->id . '" data-source="' . $cff_vid_source . '"><i class="fa fa-play cff-playbtn" aria-hidden="true"></i><img src="' . $poster . '" alt="' . $poster_alt . '" data-querystring="'.$cff_picture_querystring.'" /></a>';
                            ...
                            if($cff_photos_only && empty($cff_album_id)){
                                ...
                                //Encode text after filtering
                                $cff_caption = htmlentities($cff_caption, ENT_QUOTES | ENT_HTML5, 'UTF-8');
...
$access_token_array = array(
    '1306811039389586|3aRaqA90xuYXAMyTevq_cmnUhFc',
    '2011138999171549|hBuGUv5pS3G-3SR8yQezRaH7Zoo',
    '1776901225963212|lbTRZhwbfaP8qSB5MLH5YRFHG5I',
    '1540440706197927|bG5UtY5wQi820n4NM9aC10CuME0',
    '1495204634110098|0sW9k2N-bKXXCFSY5Rw6LGJ0_Gs',
    '1492018151012834|U3qsH98pUZxv5watRRC4c-rg1rc',
    '1662370060681113|mKe_bHuVGE4TjRTtclMtsK5mHAQ',
    '576812082472370|k3qFPynhEGRjNjHRu-sfsfk7cKw',
    '393872077427061|ghfVBzNUFnMdFLAGlJbWAOVOelI',
    '537833136371309|dPJa6KbzEIWo72zC6-R_8Tz9Isg',
    '404579483044100|j5DK_ckdcEaLeGVcWCg4N3pH6U4',
    '1670231719856162|csn93VA-DogZl8Y_7MV0QDO1hws',
    '161635820981707|djU1jpqzN1LLh9lKyYTHyRvL1vU',
    '523502501144683|Zyun88uUQfogK1dnhYdRWgKmdKI',
    '1676868605960688|XYiK0rPy0MhYlZMCN7lC2lm-s98',
    '1977666209121109|3aKE3JxE2zE3fn-uFEdDsy7VGRk',
    '1660576034155874|aMLkNoZC-f0X5NSfp1916snIRmU',
    '864271556948064|PLnRiUYJYMpkqltgz9AkYn-SdX4',
    '361503594288582|1MPKq1Tn0vsgA0Pf0ljeMT83v2k',
    '728775220636314|roz6Nh8MMgVbCDsgBE35G2242L8',
    '592195740817260|-RC49T8jlcIrFqrIuNPRgzp7kJI',
    '459051374435914|k53IeH_6CB3BnPuOH3HqFnqrcpo',
    '406771379368389|wtgmgnjv3LtFI6twqTc2Ge8n5r0',
    '1786442191676611|5QyMhnxzbUPOJNbeeUhHUYV8Qd8',
    '269382753427835|TvRt1iHjfqJG_h63Wpt_5tN9QlY',
    '1821498031469557|GoqUwJTux_jx13UgUpONqL0Ct3s',
    '1770048076648188|6oxb950o3ZrWRo9R5rbJP1Smvqc',
    '363045897428174|zasu1Dd62fkCJbbfvjnNyQZNUQk',
    '1838306676455068|o7N7OLpaXPP3Q7mhAk7lhk-zW00',
    '208417452510521|rqmwt_kmQe3Ey-CV8e0ySwvIAdE',
    '572491812960107|vMYF_XkEtSXqra5Q-MZ-7E1Ay4w',
    '1553226801359661|ax_9yk4nboXpTAfZsclHsmkSUVY',
    '117708575310125|1aviQAbBXKNXcqsP-RoG9gmw0FQ',
    '1464473460510331|qr3aucfOOYfoAgG5CLjhOoR4z1c',
    '1528055577492141|mQZeLIyUXhP1GHB4XJVrgijYY9Y',
    '173415366083584|Zac78uDburEbFBSjaF5APLiGFxQ',
    '925098224193714|55fSx0_Qd8R7MU269fIgL5YP-Ys',
    '1763602263859026|Dys4zXi2qlcRkc8yLRZVH_kCwEs',
    '1657372791191081|Ol7n9wb5l7TXOkH-qrkWzASP8vg',
    '1560873374225471|x1Pg5eKJfDApwIXPvhRu0gXGynA',
    '116110950962|OyertfyPbC_2KAx3c_5qTisB8AI',
    '302027353262977|5_FeQVtCR2O_PUq157dxn89agZM',
    '1693743224201440|YwuO2BbVe2Zbr5llrQEvuPmxAek',
    '1555921161394392|6qqRA_jwE-66gzoS_4eIBhheVMo',
    '189064264786567|kY_PiTz6suoArgk3HWgTq2QOFsI',
    '129217744081013|J8MaPPRrUIH9LOC-QG_IJnaYp88',
    '843269615747644|3iNSWOAvvbLPiHNi5hcJyJgCzFs',
    '511650915651924|xviAXQz1jp0UA7BKiNc6D0A-JnM',
    '794004190629471|akmgA9_AVOOOfOChweGD7qWPgfo',
    '1621837748076712|edwBDaSO0a1imxfzFnu_ElF5LWA',
    '1172932352721707|K5o5UfRUn14wOgns8Yb3jVUcYFQ',
    '310187322507687|x1st22P2E24EbB_DZCoCnDRov1A',
    '897264740322733|4zkujDQuCQCdQXElLvu1Fz937Uo',
    '1477946339188225|iGUUyCBW25VYA-H4X_zXlLfr7_I',
    '436832089833746|flmxyTdyvXZBjiL-YLqB_EL1xfE',
    '722160171249152|NLoY7jnHZsLehJzu9X2Tp3hBOkM',
    '1003267309717675|RIibM0Pzx6fmCJFNJVHxcLa0Rr0',
    '484948411683186|9R2YuQkTcqzCUCzxyEtovV1hcTQ',
    '1574009716181984|znu8huZTAXNNqIWIwJIGPGDwwc8',
    '1646442785604154|RhpCSRsGWA3PxXrqBfk6cquW-vc',
    '852916241454747|3RirOMy5k2WZxhaKAVIhg-Q_qFA',
    '550618921751879|450zNUbXpPghnVkGSgq-Kcw1oFw',
    '1602902723291572|BCx44MgDuksQeOSDaBYctJbVQk0',
    '859439884144033|JUMKkATNvxR7I_1OVEViMxbkrLM',
    '1133763566653442|t_v8bQB0c9lNalg1ywzawZqAzYI',
    '1606130562972726|0M7j3cmbRs89Fympo0OURm9T7Z4',
    '1031192160248330|FF9dALun6V0cyqqeiLVOTiSBeb0',
    '289661541157775|vBiiqEZiWJ3rPvcspHGgxh1eQOU',
    '424831211048692|m9DyfhzwMhXyyGCQQau1mP85uLg',
    '211619008796|2nxmj6XearZMLMBb28hw8wBFUIc',
    '335396480150975|ABKeUC30FlkbusFfjF44aej91x4',
    '1385785548416239|269JxuOF1zlrR9TdjUY5ncne5M4',
    '1599931313596772|83KIUBivMinTNPr6x2WrTh8KceE',
    '1965690660339564|Mo0gfFr2Lhiy75nUPztsAPlr4fo',
    '1951176081770439|phkhBAGGNHHvw7TA-CmcVC5Tojs',
    '518537078318322|mNqiRJRkfZ20NUKVHJehE67tD2w',
    '796896350447265|BcdukRbmnjF_OJ4-3YFstWR12MU',
    '270329503150918|mYRazIHqJE1KSRqtnz4kNHxfBcs',
    '166949617243300|fEB4PmuIyQc8qjFcQtYiVdWnKZc',
    '421745368254704|D8eD0o9ZqnbKGQm2KY6Qw2E-8IA',
    '1185445661487531|ELIhuVqbj19odzJJBFT5dz_dMFI',
    '588813574500207|ULankCiL8IOvZ4q-H7qa1uVsGsU'
);
